...
-- 2. Garantir criação da tabela public.usuarios
...
-- Habilitar RLS e criar política de leitura pública/autenticada
...
BEGIN
  -- --------------------------------------------------------------------------
  -- 1. USUÁRIO MASTER (master@hypetatu.com.br / HypeMaster@2024)
  -- --------------------------------------------------------------------------
  ...
  BEGIN
    ...
    INSERT INTO auth.identities (
      ...
      gen_random_uuid(), v_master_id::text, v_master_id,
  ...
  SET id = EXCLUDED.id, role = 'master', nome = 'Carlos Henrique', slug = 'admin', status = 'ativo';

  -- --------------------------------------------------------------------------
  -- 2. USUÁRIO RECEPÇÃO (recepcao@hypetatu.com.br / HypeRecepcao@2024)
  -- --------------------------------------------------------------------------
  ...
  BEGIN
    ...
    INSERT INTO auth.identities (
      ...
      gen_random_uuid(), v_recepcao_id::text, v_recepcao_id,
  ...
  ON CONFLICT (email) DO UPDATE 
  SET id = EXCLUDED.id, role = 'recepcionista', nome = 'Recepção Hype Tatu', slug = 'recepcao', status = 'ativo';
